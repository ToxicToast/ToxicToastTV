...
type ReceiptUseCase interface {
	...
	UpdateReceiptOCRData(ctx context.Context, id string, imagePath, ocrText string, totalPrice float64) (*domain.Receipt, error)
...
func (uc *receiptUseCase) UpdateReceiptOCRData(ctx context.Context, id string, imagePath, ocrText string, totalPrice float64) (*domain.Receipt, error) {
	receipt, err := uc.receiptRepo.GetByID(ctx, id)
	if err != nil {
		return nil, err
	}
...
func (uc *receiptUseCase) AutoMatchReceiptItems(ctx context.Context, receiptID string, similarityThreshold float64) (int, int, error) {
	...
	log.Printf("Auto-matching: Processing %d items for receipt %s", len(items), receiptID)
	...
	for _, item := range items {
		...
		if bestMatch != nil {
			...
			log.Printf("  NO MATCH for '%s' (best similarity: %.2f, threshold: %.2f)", item.ItemName, bestSimilarity, similarityThreshold)
			unmatchedCount++
		}
	}
...
func calculateSimpleSimilarity(s1, s2 string) float64 {
	...
	// Normalize strings: remove dots, convert to lowercase
	...
	// Count matching words and partial matches
	...
	for _, w1 := range words1 {
		for _, w2 := range words2 {
			...
			// Partial match: one word starts with or contains the other
			if len(w1) >= 3 && len(w2) >= 3 {
				if strings.HasPrefix(w2, w1) || strings.HasPrefix(w1, w2) {
					matchingWords++
					break
				}
				// Check if w1 is contained in w2 or vice versa (for abbreviations)
